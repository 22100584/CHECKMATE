...
import { getMyPage, uploadImage } from "../apis/mypage";
...
function MyPost() {
  ...
  const onChangeImage = (event) => {
    if (event.target.files && event.target.files[0]) {
      ...
      uploadImage(event.target.files[0], userId).then((res) => {
        console.log(res);
